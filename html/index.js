...
function UpdateK9Name() {
    $.post("http://k9-update/recievek9name", JSON.stringify({
        name: k9_app.dogname,
    }));
};
...
function TriggerK9Search() {
    $.post("http://k9-update/triggerk9search", JSON.stringify({}));
}

// K9 Vehicle in/out (NOT WORKING)
function ToggleK9Vehicle() {
    $.post("http://k9-update/triggervehicletoggle", JSON.stringify({}));
}

// Spawn / Delete K9 (WORKS)
function ToggleK9Spawn() {
    $.post("http://k9-update/triggertogglek9", JSON.stringify({}));
};

// Disable k9 Menu (WORKS)
function DisableK9Menu() {
    $.post("http://k9-update/disablek9menu", JSON.stringify({}));
};


// K9 Play Animation (NOT IN)
function PlayK9Animaton(name) {
    $.post("http://k9-update/triggerk9animation", JSON.stringify({
        anim: name,
    }));
};
